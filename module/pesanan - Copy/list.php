...
if (mysqli_num_rows($queryPesanan) == 0) {
    ...
    <div class=''>
        ...
        <div class='items-center'>
            <button class=' mx-auto bg-indigo-500 px-4 py-2 rounded-lg font-semibold text-white'>Belanja Sekarang</button>
        </div>
    </div>";
} else {
...
<tr>
    ...
    while ($row = mysqli_fetch_assoc($queryPesanan)) {
        if ($level == "superadmin") {
            $adminButton = "<a class='ml-5 items-center px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-md' href='" . BASE_URL . "index.php?page=my_profile&module=pesanan&action=status&pesanan_id=$row[pesanan_id]'>Update Status</a>";
        }
