<h3 class="text-center text-3xl font-bold my-10">Unisex Section</h3>

<div class="">
    <hr>
    <div class="items-center flex justify-center">
        <a class="mr-28 my-10 text-xl font-bold hover:text-blue-500" href="<?php echo BASE_URL . "index.php?page=men-section"; ?>">Men Section</a>
        <a class="mr-28 my-10 text-xl font-bold text-blue-500" href="<?php echo BASE_URL . "index.php?page=unisex-section"; ?>">Unisex Section</a>
...
//     } else {
//         echo "
//          <a href='" . BASE_URL . "index.php?page=unisex-section?kategori_id=$row[kategori_id]' class='ml-1 bg-black  font-semibold text-white px-6 py-2 rounded-lg hover:bg-slate-600 text-lg'>$row[kategori]</a>
//          ";
//     }
// }
?>
<div class="mt-10 ml-5 flex">
    ...
    <li class='text-sm my-5 font-semibold'>
        <a class='' href='" . BASE_URL . "index.php?page=unisex-section&kategori_id=$row[kategori_id]' class=''>$row[kategori]</a>
    </li>
    <hr>
    ";
                        } else {
                            ...
                            "
                        <li class='text-sm my-5 font-semibold'>
     <a href='" . BASE_URL . "index.php?page=unisex-section&kategori_id=$row[kategori_id]' class=''>$row[kategori]</a>
     </li> <hr>
     ";
                        }
                    }
    ...
    <div class="frame-barang mt-15">
        <div class="p-10 flex flex-wrap items-center justify-center">
            <?php
            if ($kategori_id) {
                $query = mysqli_query($koneksi, "SELECT * FROM barang WHERE status='on' AND section='unisex' AND kategori_id='$kategori_id' ORDER BY rand() DESC LIMIT 5");
            } else {
                $query = mysqli_query($koneksi, "SELECT * FROM barang WHERE status='on' AND section='unisex' ORDER BY rand() DESC LIMIT 5");
            }
            $no = 1;
            while ($row = mysqli_fetch_assoc($query)) {
                $style = false;
